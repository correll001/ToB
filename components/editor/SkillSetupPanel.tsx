...
import type { SkillTabExplanation, SkillTabSupportLinkExplanation } from '@/types/skillTabExplanation'
...
import { selectSkillSetupPlayerView } from '@/selectors/skillSetupPlayerView'
import {
  inferredCombatRoleZh,
  SKILL_SETUP_COMBAT_ROLE_LABEL,
  SKILL_SETUP_FAILED_SKILL_NOTE,
  SKILL_SETUP_PAGE_TITLE,
  SKILL_SETUP_PASSIVE_SECTION_BLURB,
  SKILL_SETUP_PASSIVE_SECTION_TITLE,
  SKILL_SETUP_SCOPE_HINT,
  SKILL_SETUP_SLOT_DISABLED_NOTE,
  skillFamilyUiLabel,
  skillSetupNormWarningsHint,
} from '@/components/editor/skill-setup/skillSetupCopy'
import { SkillSummaryCard } from '@/components/editor/skill-setup/SkillSummaryCard'
import { SupportResultsCard } from '@/components/editor/skill-setup/SupportResultsCard'
import { ContributionFlowCard } from '@/components/editor/skill-setup/ContributionFlowCard'
import { AdvancedDetailsCard } from '@/components/editor/skill-setup/AdvancedDetailsCard'

/** PoB-style link group size cap for this editor (matches common 6L). */
const MAX_SKILL_SUPPORT_LINKS = 6

function SupportLinkExplanationPanel({ linkExp }: { linkExp: SkillTabSupportLinkExplanation }) {
  return (
    ...
    <div className="mt-4 border-t border-slate-800/70 pt-4">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <span className="text-xs font-bold uppercase tracking-wide text-slate-500">
          {isInspected ? '連結編輯與說明' : `連結輔助（1–${MAX_SKILL_SUPPORT_LINKS} 格）`}
        </span>
        {skillRow.skillId ? (
          <button
            type="button"
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 text-[11px] text-slate-500">
                <span className="font-mono text-slate-600">連結 {idx + 1}</span>
              ...
              <div className="min-w-0 flex-1 text-[10px] leading-snug sm:basis-full">
                {!skillRow.skillId ? (
                  ...
                  <span className="text-slate-600">已關閉，不會參與計算</span>
                ) : att ? (
                  <span className={att.applied ? 'text-emerald-300/90' : 'text-amber-200/85'}>
                    {att.applied ? '✓ 已套用' : '✗ 未套用'}
                    {!att.applied ? ` · ${formatSupportSkipReasonZh(att.skipReason)}` : ''}
                    {att.applied && att.warnings?.length ? ` · ${att.warnings.join('；')}` : ''}
                  </span>
                ) : (
                  <span className="text-slate-600">尚無相容結果（可能不是輔助或已被略過）</span>
                )}
              </div>
...
export default function SkillSetupPanel() {
  ...
  const { inspectedPresentationMode } = useBuildComputedStats()
  const skillTabExplanation = React.useMemo(() => selectSkillTabExplanation(snapshot), [snapshot])
  const skillSetupPlayerView = React.useMemo(() => selectSkillSetupPlayerView(snapshot), [snapshot])
  ...
  const inspectedApplied = skillSetupPlayerView.supportResults.appliedCount
  const inspectedSkipped = skillSetupPlayerView.supportResults.skippedCount
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <div className="rounded-2xl border border-slate-800/55 bg-gradient-to-b from-slate-900/35 to-slate-950/20 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        <div className="flex flex-wrap items-center gap-2">
          <span
            ...
            aria-hidden
          />
          <h2 className="text-base font-bold tracking-tight text-slate-100 md:text-lg">{SKILL_SETUP_PAGE_TITLE}</h2>
        </div>

        <p className="mt-2 text-[10px] leading-relaxed text-slate-500">{SKILL_SETUP_SCOPE_HINT}</p>

        <div className="mt-3 rounded-lg border border-slate-800/80 bg-black/25 px-3 py-2.5 pl-4">
          <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">正在預覽</div>
          <div className="mt-1 text-sm text-slate-200">
            {inspectedMainSkillSlot != null ? (
              <>
                <span className="font-mono text-violet-300">第 {inspectedMainSkillSlot} 槽</span>
                <span className="text-slate-500"> · </span>
                <span>{inspectedName}</span>
                <span className="ml-2 text-[10px] tabular-nums text-slate-500">
                  輔助生效 {inspectedApplied} · 未生效 {inspectedSkipped}
                </span>
              </>
            ) : (
              <span className="text-slate-500">尚未選槽 — 點主技能卡的「檢查此技能」或下方數字</span>
            )}
          </div>
        ...
        return (
          <div
            ...
            className={`rounded-xl border bg-slate-950/40 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] md:p-5 ${
              isInspected ? 'border-violet-600/50 ring-1 ring-violet-500/25' : 'border-slate-800/70'
            }`}
          >
            <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0 flex-1 space-y-2">
                ...
                {norm?.warnings?.length ? (
                  <p className="text-[10px] text-amber-200/90">{skillSetupNormWarningsHint(norm.warnings.length)}</p>
                ) : null}
                {parseStatus === 'failed' ? (
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              ...
              {dmgRole ? (
                <span className="text-[11px] text-slate-500">
                  {SKILL_SETUP_COMBAT_ROLE_LABEL}：<span className="text-slate-400">{inferredCombatRoleZh(dmgRole)}</span>
                </span>
              ) : null}
            </div>

            {skillRow.enabled === false ? (
              <p className="mt-3 text-[11px] text-slate-500">{SKILL_SETUP_SLOT_DISABLED_NOTE}</p>
            ) : null}

            {isInspected ? (
              <div className="mt-4 space-y-4">
                <SkillSummaryCard
                  summary={skillSetupPlayerView.skillSummary}
                  parseStatus={skillTabExplanation.parseStatus}
                />
                <SupportResultsCard results={skillSetupPlayerView.supportResults}>
                ...
                <ContributionFlowCard flow={skillSetupPlayerView.contributionFlow} />
                <AdvancedDetailsCard
                  advanced={skillSetupPlayerView.advancedDetails}
                  explanation={skillTabExplanation}
                  snapshot={snapshot}
                  inspectedPresentationMode={inspectedPresentationMode}
                  inspectedActiveSkillId={skillTabExplanation.activeSkillId}
                  mainSkillDefinitionTags={tagChips}
                  mainSkillNormWarnings={norm?.warnings ?? []}
                />
              </div>
            ) : null}
      ...
      <div className="rounded-2xl border border-slate-800/55 bg-gradient-to-b from-slate-900/35 to-slate-950/20 p-5">
        <div className="mb-3 text-base font-semibold text-slate-100">{SKILL_SETUP_PASSIVE_SECTION_TITLE}</div>
        <p className="mb-3 text-xs text-slate-500">{SKILL_SETUP_PASSIVE_SECTION_BLURB}</p>
            ...
            return (
              <div key={p.slot} className="rounded-lg border border-slate-800/50 bg-black/15 p-3">
                <div className="flex flex-wrap items-end gap-2">
                  <div className="min-w-[8rem] text-xs text-slate-500">被動槽 {p.slot}</div>
                  <select
                    value={p.skillId ?? ''}
                ...
                {p.skillId ? (
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    {pdef ? (
                      ...
                      >
                        {skillFamilyUiLabel(pdef.family)}
                      </span>
                    ) : null}
                  </div>
                ) : null}
                ...
                {pNorm?.warnings?.length ? (
                  <p className="mt-2 text-[10px] text-amber-200/85">
                    {skillSetupNormWarningsHint(pNorm.warnings.length)}
                  </p>
                ) : null}
